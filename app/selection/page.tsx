...
export default function SelectionPage() {
  ...
  useEffect(() => {
    if (!loading) {
      ...
      // Already has course → skip selection, go dashboard
      if (userData?.course) {
        localStorage.setItem("eduNexCourse", userData.course);
    ...
    <>
      <style>{`
        ...
        *{box-sizing:border-box;margin:0;padding:0;}
        ...
        .bg-glow{position:fixed;inset:0;pointer-events:none;z-index:0;
          background:radial-gradient(ellipse 80% 50% at 50% -10%, ${dm?"rgba(79,70,229,0.15)":"rgba(79,70,229,0.07)"} 0%, transparent 70%);}
        ...
        .nav-user{display:flex;align-items:center;gap:8px;padding:6px 12px;background:var(--surface);border:1px solid var(--border);border-radius:100px;font-size:0.8rem;font-weight:600;color:var(--sub);}
        .nav-avatar{width:24px;height:24px;border-radius:50%;background:linear-gradient(135deg,#4f46e5,#0d9488);display:flex;align-items:center;justify-content:center;font-size:0.7rem;font-weight:800;color:white;overflow:hidden;flex-shrink:0;}
        .nav-avatar img{width:100%;height:100%;object-fit:cover;}
        .logout-btn{padding:6px 14px;background:transparent;border:1px solid var(--border);border-radius:100px;font-size:0.78rem;font-weight:600;color:var(--sub);cursor:pointer;transition:0.2s;}
        .logout-btn:hover{border-color:rgba(239,68,68,0.4);color:#ef4444;}
        .theme-btn{width:34px;height:34px;border-radius:50%;background:var(--surface);border:1px solid var(--border);cursor:pointer;display:flex;align-items:center;justify-content:center;font-size:0.9rem;}
        ...
        .welcome-badge{display:inline-flex;align-items:center;gap:8px;padding:6px 16px;border-radius:100px;background:${dm?"rgba(79,70,229,0.12)":"rgba(79,70,229,0.08)"};border:1px solid ${dm?"rgba(79,70,229,0.25)":"rgba(79,70,229,0.15)"};font-size:0.78rem;font-weight:700;color:#818cf8;margin-bottom:20px;}
        .header h1{font-family:'Bricolage Grotesque',sans-serif;font-size:clamp(1.8rem,5vw,2.8rem);font-weight:800;line-height:1.1;margin-bottom:12px;letter-spacing:-0.02em;}
        ...
        .continue-btn{
          ...
          background:${selected?"linear-gradient(135deg,#4f46e5,#0d9488)":"var(--surface2)"};
          color:${selected?"white":"var(--sub)"};
          ...
          <div className="nav-user">
            <div className="nav-avatar">
              {userData?.photo || user?.photoURL
                ? <img src={userData?.photo || user?.photoURL || ""} alt="avatar" />
                : (firstName?.[0] ?? "S").toUpperCase()}
            </div>
            {firstName}
          </div>
          <button className="theme-btn" onClick={() => { const n = !darkMode; setDarkMode(n); localStorage.setItem("eduNexDark", String(n)); }}>
            {dm ? "☀️" : "🌙"}
          </button>
          <button className="logout-btn" onClick={async () => { await logOut(); window.location.href = "/"; }}>
            Logout
          </button>
        </div>
      </nav>
      ...
      <div className="page">
        <div className="header">
          <div className="welcome-badge">👋 Welcome to EduNex AI</div>
          <h1>Hey {firstName}!<br/>Select Your <span className="grad">Course</span></h1>
          <p>Choose your class or course so we can show you the right study materials, notes and PYQ papers.</p>
        </div>
        ...
        <div className="continue-wrap">
          ...
          >
            {saving
              ...
              : selected
                ? <>Continue as {COURSES.find(c=>c.id===selected)?.icon} {selected} Student →</>
                : "Select a course to continue"}
          </button>
          <div className="step-hint">You can change this later from your profile settings</div>
        </div>
      </div>
    </>
  );
